...
if (length(commandArgs(trailingOnly = TRUE)) != 6) {
  stop("Please provide seven command line arguments: Input path, SNP file path, SNP column label, SNP type (single or pair), sex column label, target column label, and output path")
}


input_file <- commandArgs(trailingOnly = TRUE)[1]
snps_file <- commandArgs(trailingOnly = TRUE)[2]
snp_column <- commandArgs(trailingOnly = True)[3]
curr_snp <- commandArgs(trailingOnly = TRUE)[4]
sex_column <- commandArgs(trailingOnly = TRUE)[5]
phq9_column <- commandArgs(trailingOnly = TRUE)[6]
output_path <- commandArgs(trailingOnly = TRUE)[7]
...
for (i in seq_len(nrow(filtered_snps))) {
  ...
  my_data <- data[, c(snp_column, sex_column, phq9_column)]
  srh_formula <- as.formula(paste(phq9_column, "~", snp_column, "+", sex_column))
  
  srh_results <- scheirerRayHare(formula = srh_formula, data = my_data)
  
  # Build the output file name
  if (curr_snp == "single") {
    gene_name <- as.character(filtered_snps[i, "Gene"])
    output_file <- sprintf("./results/%s_%s_srh.csv", gene_name, snp_name)
  } else {
    gene1_name <- as.character(filtered_snps[i, "Gene1"])
    gene2_name <- as.character(filtered_snps[i, "Gene2"])
    output_file <- sprintf("./results/%s_%s_%s_srh.csv", gene1_name, gene2_name, snp_name)
  }
  
  write.csv(list(Scheirer_Ray_Hare = srh_results), file = output_path)
}