...
# install.packages("SNPassoc") # nolint
...
snpdata <- read.csv("SNP_assoc_testing.csv")
...
column_range <- paste0("[", min(snp_column_numbers), ":", max(snp_column_numbers), "]") # nolint
...
clinical <- c("Sex", "Age", "Chronotype", "Sleeplessness.Insomnia")
target <- "PHQ9_binary"

snpcols <- colnames(snpdata)[1:10]
...
write.csv(result.snp, file = "./data/snpAssocResults/snp-snp_interaction.csv", row.names = TRUE) # nolint
png("./data/snpAssocResults/snp-snp_interaction.png", width = 8, height = 6, units = "in", res = 300)  # Adjust the resolution as needed # nolint
